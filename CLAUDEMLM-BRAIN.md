...
Latest nordic-vitals push: 2026-07-27 (commit c47e100) — Admin Referral Tracking page: /admin/referrals (KPI cards: active referrers/clicks30d/conversions30d/avg-rate; sortable table: member, referral code, clicks, conversions, conv-rate bar, lifetime stats, commissions, last conversion; Top-10 bar chart via recharts; search+rank+status filters; CSV export; pagination). REFERRAL_STATS mock dataset (10 members). getAdminReferrals() mock-safe in mlmApi.js. AdminLayout Referrals nav + Overview quick-nav updated. App.jsx route wired. Vite build passes. Board-watcher run 57: board still inaccessible, no new Bjorn tasks.
