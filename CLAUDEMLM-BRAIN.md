...
Latest nordic-vitals push: 2026-07-28 (commit 33b17ea) — Rank Progress page (/dashboard/rank-progress): visual rank ladder (Unranked→Bronze→Silver→Gold→Platinum), per-metric progress bars to next rank (PV/Left GV/Right GV/Active Recruits), current rank perks, tips for reaching next rank, all-rank requirements table, rank history timeline. getRankProgress() added to mlmApi.js (mock-safe, wires to GET /v1/mlm/members/:userId/rank-progress). RANK_HISTORY mock dataset in mock.js. Nav link added to DashboardLayout. Vite build passes ✅. Board-watcher run 61 (complete): board still inaccessible; no new Bjørn emails (last: "Dagens Hacker News-svar" 2026-07-27 — still Gary-manual).
