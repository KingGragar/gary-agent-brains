...
Latest nordic-vitals push: 2026-07-27 (commit fb8a3b7) — Admin Overview: added Recent Signups panel (last 5 members by join date, with country/status) and Near Rank Up panel (active members ranked by min(PV%,GV%) progress toward next rank threshold, colour-coded progress bar: green ≥80%, gold ≥50%, blue <50%; shows current→next rank, raw PV/GV). Both panels use already-loaded members state — no extra API calls. Vite build passes. Board-watcher run 58: board still inaccessible, no new Bjorn tasks.
