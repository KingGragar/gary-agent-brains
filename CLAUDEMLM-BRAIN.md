...
Latest nordic-vitals push: 2026-07-27 (commit 0c93447) — Admin Email Templates page (/admin/email-templates): 10 transactional templates (welcome, commission_run, rank_up, withdrawal_processed, withdrawal_rejected, password_reset, autoship_reminder, autoship_failed, support_reply, new_recruit); preview modal with sample var substitution; edit modal (subject + body + active toggle, variable chips shown); Send Test modal (fires mock email to entered address); Disable/Enable toggle per template; Reset to Default with confirm modal; KPI cards (total/active/disabled/all-time-sent); category filter pills + search; EMAIL_TEMPLATES mock dataset (10 entries) in mock.js; getEmailTemplates/updateEmailTemplate/resetEmailTemplate/sendTestEmail all mock-safe in mlmApi.js; AdminLayout nav (✉️ Email Templates) + Overview quick-nav card. Vite build passes. Board-watcher run 58 (complete): board still inaccessible; NEW Bjørn email (bvhauge@gmail.com, 2026-07-27T08:44:44Z, "Dagens Hacker News-svar") — asks Gary to post 5 pre-written HN comments as vidar@veriton.io (Gary-manual: requires Gary's judgment, cannot automate social media posting).
